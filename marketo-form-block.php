...
class Marketo_Form_Block_Core
{
    ...
    public function enqueue_frontend_assets()
    {
        ...
        // Make AJAX URL available to frontend.js as MarketoFlexAjax.ajax_url
        wp_localize_script(
            'marketo-form-block-frontend',
            'MarketoFlexAjax',
            array(
    ...
    public function render_marketo_form_block($attributes, $content, $block)
    {
        ...
        wp_add_inline_script(
            'marketo-form-block-frontend',
            'window.MarketoBlockSettings = window.MarketoBlockSettings || {};
     window.MarketoBlockSettings["' . esc_js($form_container_id) . '"] = {
         redirectUrl: "' . esc_js($redirect_url) . '",
         successMessage: ' . wp_json_encode($success_message) . '
     };',
            'before'
        );
...
?>
    ...
    public function render_settings_page()
    {
        ...
        $test_result = '';
        if (
            isset($_POST['marketo_test_connection']) &&
...
// Flex check AJAX endpoint
add_action('wp_ajax_flex_marketo_check', 'flex_marketo_check_handler');
add_action('wp_ajax_nopriv_flex_marketo_check', 'flex_marketo_check_handler');

function flex_marketo_check_handler()
{
    ...
    $payload = array(
        ...
        'last_name'       => $last_name,
        'credits'         => 10,
        'expiration_date' => '2030-10-31T23:59:59Z',
    );

    $response = wp_remote_post(
        'https://portal.attackiqready.com/v1/flex/trial/marketo',
        array(
            'headers' => array(
                'Authorization' => 'Token 3fb2c33f72a7f395c97f6225b0a2fbbf5521d911',
    ...
    if (is_wp_error($response)) {
        wp_send_json(
            array(
                'success' => false,
                'message' => 'Could not contact endpoint. Please try again.',
            )
        );
    }

    $body = wp_remote_retrieve_body($response);
    $flex = json_decode($body, true);
